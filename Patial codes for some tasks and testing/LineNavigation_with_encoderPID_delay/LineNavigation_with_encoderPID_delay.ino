...
void LineNavigation(){
  ...
  if (current_line==0 && new_line==1){ //enter into white strip
    No_lines++;
    motor1.stop();
    motor2.stop();
    delay(500);
    ...
    //encoder_pulse=encoderCountA1;
    Serial.print("x value: ");
    Serial.print(encoderCountA1);
  ...
  //     increment_count++;
  //     for (int j=0; j<(No_lines-3) ; j++){
  //       //Serial.print(Final_Code[0]);
  //       //Serial.print("\t");
  //     }
...
bool Identify_Strip(int* sensorValues){
  readSensors(sensorValues);
  if ((sensorValues[0]+sensorValues[1]+sensorValues[2]+sensorValues[3]+sensorValues[4]+sensorValues[5]+sensorValues[6]+sensorValues[7])>1){
